...
def recursion_phase(vrchol, cas, pozice):
    ...
    for udalost in pomocne_pole_udalosti:
        ...
        if (doba > maximum):
            maximum = doba
            rekordni_pozice = index
            vrchol_maxima = novy_vrchol
    return maximum, (cas, vrchol_maxima, rekordni_pozice)
...
def rekonstrukce_trasy(aktualni_udalost):
    trasa = []
    while(True):
        cas, mistnost, aktualni_index = aktualni_udalost
        trasa.append((cas, mistnost))
        if (aktualni_index == None):
            return trasa

        aktualni_udalost = pomocne_pole[aktualni_index][1]
...
trasa = rekonstrukce_trasy(trasa)
